...
if (!empty($selected_subs)) {
    ...
    foreach ($selected_subs as $s) {
        if ($s === "Men's Wear") {
            $sub_clauses[] = "(p.subcategory = 'Men\'s Wear' OR p.subcategory = 'Men\'\'s Wear' OR p.subcategory = 'Menswear' OR p.subcategory = 'Men Wear' OR p.subcategory LIKE '%Men%Wear%' OR ((p.title LIKE '%Men\'s%' OR p.title LIKE '%Mens%' OR p.title LIKE '%Men%Shirt%' OR p.tags LIKE '%Men%') AND p.title NOT LIKE '%Women%'))";
