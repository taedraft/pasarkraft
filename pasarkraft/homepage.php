...
function pk_run_python_recommender($userId, $limit) {
...
<body>
    ...
    <header>
        <nav>
            ...
            <form class="header-search" method="GET" action="homepage.php">
                <input type="text" name="q" placeholder="Search by product name, color, pattern..." value="<?php echo htmlspecialchars($filter_query); ?>">
                ...
                <div class="filter-dropdown" id="filterDropdown">
                    <div class="filter-section">
                        ...
                        <select name="category" class="filter-input">
                            ...
                            <option value="Batik" <?php echo $filter_category === 'Batik' ? 'selected' : ''; ?>>Batik</option>
                            <option value="Woodcraft" <?php echo $filter_category === 'Woodcraft' ? 'selected' : ''; ?>>Woodcraft</option>
                        </select>
                    </div>
                    <div class="filter-section">
                        <h4>Type & Pattern</h4>
                    ...
                    <div class="filter-section">
                        ...
                        <div style="margin-top: 8px; font-size: 0.8rem; color: #7f8c8d;">Selected: <span id="filterColorLabel"><?php echo $filter_color !== '' ? htmlspecialchars($filter_color) : 'Any'; ?></span></div>
                    </div>
                    <div class="filter-section">
    ...
    <section id="products" class="products">
        ...
        <?php else: ?>
        <div class="no-products-found" style="text-align: center; padding: 4rem 2rem; background: #fff; border-radius: 12px; border: 1px dashed #ddd; max-width: 600px; margin: 0 auto 3rem;">
            <i class="fas fa-search" style="font-size: 3rem; color: #bdc3c7; margin-bottom: 1rem; display: block;"></i>
            <h3 style="font-size: 1.5rem; color: var(--primary-color); margin-bottom: 0.5rem; font-family: var(--font-heading);">No Products Found</h3>
            <p style="color: #7f8c8d; margin-bottom: 1.5rem; font-size: 0.95rem;">We couldn't find any products matching your search or filters. Try adjusting your selections or clearing the filters.</p>
            <a href="homepage.php" class="btn" style="display: inline-block; padding: 10px 24px; text-decoration: none; border-radius: 6px; font-weight: 500;">Clear Filters</a>
        </div>
        <?php endif; ?>
    </section>
    ...
    <div id="pk-chatbot" class="pk-chatbot">
        ...
        <div class="pk-chatbot-panel" hidden>
            ...
            <div class="pk-chatbot-body" id="pk-chatbot-body">
                <div class="pk-chatbot-message bot">Hi! I am the PasarKraft AI Guide. Ask me anything about Batik, Woodcraft, or how to contact our artisans!</div>
            </div>
            ...
            <div class="pk-chatbot-input-area" style="display: flex; border-top: 1px solid #eee; padding: 8px 12px; gap: 8px; background: #fff;">
                <input type="text" id="pk-chatbot-input" placeholder="Type a message..." style="flex: 1; border: 1px solid #ddd; padding: 6px 12px; border-radius: 20px; font-size: 0.85rem; outline: none; font-family: var(--font-body);">
                <button type="button" id="pk-chatbot-send" style="background: #2c3e50; color: #fff; border: none; width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: background 0.2s;"><i class="fas fa-paper-plane" style="font-size: 0.8rem;"></i></button>
            </div>
        </div>
    </div>
...
<style>
    .pk-chatbot { position: fixed; right: 24px; bottom: 24px; z-index: 50; font-family: var(--font-body); }
    .pk-chatbot-toggle { display: flex; align-items: center; gap: 8px; background: #2c3e50; color: #fff; border: none; padding: 12px 16px; border-radius: 999px; cursor: pointer; box-shadow: 0 10px 20px rgba(0,0,0,0.2); }
    .pk-chatbot-panel { width: 300px; background: #fff; border: 1px solid #eee; border-radius: 12px; box-shadow: 0 12px 24px rgba(0,0,0,0.12); margin-top: 12px; overflow: hidden; }
    .pk-chatbot-header { display: flex; justify-content: space-between; align-items: center; padding: 12px 14px; background: #f8f4ef; }
    .pk-chatbot-header button { background: none; border: none; font-size: 1.2rem; cursor: pointer; }
    .pk-chatbot-body { padding: 12px 14px; height: 240px; overflow-y: auto; display: flex; flex-direction: column; gap: 8px; }
    .pk-chatbot-message { padding: 8px 12px; border-radius: 12px; font-size: 0.85rem; line-height: 1.4; word-break: break-word; }
    .pk-chatbot-message.bot { background: #f1f5f9; color: #2c3e50; align-self: flex-start; }
    .pk-chatbot-message.user { background: #2c3e50; color: #fff; align-self: flex-end; }
    .pk-chatbot-message.typing { font-style: italic; color: #7f8c8d; background: #f1f5f9; align-self: flex-start; }
    .pk-chatbot-actions { display: grid; gap: 8px; padding: 12px 14px; border-top: 1px solid #eee; background: #faf9f6; }
    .pk-chatbot-actions button { padding: 8px; border-radius: 8px; border: 1px solid #ddd; background: #fff; cursor: pointer; font-size: 0.8rem; text-align: left; transition: all 0.2s; }
    .pk-chatbot-actions button:hover { border-color: #2c3e50; background: #fdfaf6; }
    .pk-chatbot-input-area button:hover { background: var(--accent-color, #d35400) !important; }
    @media (max-width: 640px) { .pk-chatbot-panel { width: 90vw; } }
</style>
